...
import com.yash.employeeInfogram.util.HibernateUtil;
...
public class EmpServiceImpl implements EmpServices {
	...
	public boolean addEmployee(Employee emp) {
		...
		try {
			...
			HibernateUtil.resourceCleanUp(session, tx);
		}
		return false;
	}
	...
	public List<Employee> getAllEmployees() {
		 ...
		 * list.forEach(System.out::println); consumer.accept(list);
		 */
		return null;
	}
	...
	public boolean getEmployeeById(int id) throws ParseException {
		...
		try {
			...
			//getAllEmployees().stream().filter((e) -> e.getId() == id).forEach(System.out::println);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public boolean getEmployeeByName(String name) throws ParseException {
		...
		try {
			getAllEmployees().stream().filter((e) -> e.getName().equalsIgnoreCase(name)).forEach(System.out::println);
			return true;
		} catch (Exception e) {
	...
	public boolean getEmployeeBySalary(double salary, int option) throws ParseException {
		...
		case 1:
			...
			List<Employee> empList = getAllEmployees().stream().filter(predicate).collect(Collectors.toList());
			if (!empList.isEmpty()) {
